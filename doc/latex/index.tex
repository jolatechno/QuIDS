\label{index_md__home_joseph_Documents_projets_programing_cpp_Quantum_graph_simulation_src_QuIDS_README}%
\Hypertarget{index_md__home_joseph_Documents_projets_programing_cpp_Quantum_graph_simulation_src_QuIDS_README}%
...
The only requirement is to have at least {\ttfamily c++2a}. Parallelism is implemented using {\ttfamily Open\+MP}, although the pure {\ttfamily MPI} implementation is more efficient right now.\hypertarget{index_autotoc_md3}{}\doxysection{Usage}\label{index_autotoc_md3}
...
\end{DoxyCode}
\hypertarget{index_autotoc_md4}{}\doxysubsection{MPI support}\label{index_autotoc_md4}
...
\DoxyCodeLine{    MPI\_Init\_thread(\&argc, \&argv, MPI\_THREAD\_MULTIPLE, \&provided);}
\DoxyCodeLine{    \textcolor{keywordflow}{if}(provided < MPI\_THREAD\_MULTIPLE) \{}
...
\end{DoxyCode}
\hypertarget{index_autotoc_md5}{}\doxysubsection{Modifiers}\label{index_autotoc_md5}
...
\end{DoxyCode}
\hypertarget{index_autotoc_md6}{}\doxysubsection{Rules}\label{index_autotoc_md6}
...
\end{DoxyItemize}\hypertarget{index_autotoc_md9}{}\doxysubsubsection{Iteration}\label{index_autotoc_md9}
...
\end{DoxyItemize}\hypertarget{index_autotoc_md10}{}\doxysubsubsection{MPI symbolic iteration}\label{index_autotoc_md10}
...
\end{DoxyItemize}\hypertarget{index_autotoc_md11}{}\doxysubsubsection{MPI iteration}\label{index_autotoc_md11}
...
{\ttfamily node\+\_\+total\+\_\+proba} is the only additional member variable, and is the proportion of total probability that is held by a given node.\hypertarget{index_autotoc_md12}{}\doxysubsection{Global parameters and pre-\/processor flags}\label{index_autotoc_md12}
...
\end{DoxyCode}
\hypertarget{index_autotoc_md13}{}\doxysubsection{Pre-\/processor flags}\label{index_autotoc_md13}
\hypertarget{index_autotoc_md14}{}\doxysubsubsection{proba. type}\label{index_autotoc_md14}
The {\ttfamily PROBA\+\_\+\+TYPE} flag is used to change the type used to represent probabilities and magnitude in the program. The default is {\ttfamily double} for a high enough precision, but other float type could be used to either get more precision or less memory/execution time cost.\hypertarget{index_autotoc_md15}{}\doxysubsubsection{hash map overhead}\label{index_autotoc_md15}
The {\ttfamily HASH\+\_\+\+MAP\+\_\+\+OVERHEAD} flag represent the overhead per element of the hashmap (of type {\ttfamily robin\+\_\+hood\+::unordered\+\_\+map}, default is set to {\ttfamily 1.\+7} which has been determined through experiments).\hypertarget{index_autotoc_md16}{}\doxysubsection{Global variables}\label{index_autotoc_md16}
The default value of any of those variable can be altered at compilation, by passing an uppercase flag with the same name as the desired variable.\hypertarget{index_autotoc_md17}{}\doxysubsubsection{tolerance}\label{index_autotoc_md17}
{\ttfamily tolerance} represents the minimum probability considered non-\/zero (default is {\ttfamily 1e-\/30}, to compensate for numerical errors).\hypertarget{index_autotoc_md18}{}\doxysubsubsection{safety margin}\label{index_autotoc_md18}
{\ttfamily safety\+\_\+margin} represents the target proportion of memory to keep free (default is {\ttfamily 0.\+1} for 10\%).\hypertarget{index_autotoc_md19}{}\doxysubsubsection{simple truncation}\label{index_autotoc_md19}
...
Probabilistic selctions cost a bit of time and of accuracy, with some gain in representation through an analog sampling process to a quantum Monte-\/\+Carlo algorithm.\hypertarget{index_autotoc_md20}{}\doxysubsubsection{load balancing bucket per thread}\label{index_autotoc_md20}
...
{\ttfamily load\+\_\+balancing\+\_\+bucket\+\_\+per\+\_\+thread} has a default of {\ttfamily 8}.\hypertarget{index_autotoc_md21}{}\doxysubsection{MPI global variables}\label{index_autotoc_md21}
\hypertarget{index_autotoc_md22}{}\doxysubsubsection{minimum equalize size and equalize imbalance.}\label{index_autotoc_md22}
...
{\ttfamily mpi\+::min\+\_\+equalize\+\_\+size} is equal to {\ttfamily 1000} by default, and {\ttfamily mpi\+::equalize\+\_\+imablance} has a default of {\ttfamily 0.\+2}.\hypertarget{index_autotoc_md23}{}\doxysubsection{Utils global variables}\label{index_autotoc_md23}
\hypertarget{index_autotoc_md24}{}\doxysubsubsection{min vector size}\label{index_autotoc_md24}
{\ttfamily utils\+::min\+\_\+vector\+\_\+size} represent the minimum size of any vector (the default is {\ttfamily 100000}).\hypertarget{index_autotoc_md25}{}\doxysubsubsection{upsize policy}\label{index_autotoc_md25}
{\ttfamily utils\+::upsize\+\_\+policy} represent the multiplier applied when upsizing a vector (the default is {\ttfamily 1.\+1}). It avoid frequent upsizing by giving a small margin.\hypertarget{index_autotoc_md26}{}\doxysubsubsection{downsize policy}\label{index_autotoc_md26}
